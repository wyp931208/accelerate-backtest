    ...
    with st.expander("⚙️ 回测参数设置", expanded=True):
        ...
        with col3:
            ...
            cum_pct_min = st.number_input(
                "累计涨幅下限(%)", min_value=-100.0, max_value=100.0,
                value=0.0, step=5.0, key="bt_cum_min",
                disabled=not require_cum_pct
            )
            cum_pct_max = st.number_input(
                "累计涨幅上限(%)", min_value=-100.0, max_value=100.0,
...
with tab2:
    ...
    with st.expander("⚙️ 信号参数设置", expanded=False):
        ...
        with col_s2:
            ...
            sig_cum_max = st.number_input("累计涨幅上限(%)", min_value=-100.0, max_value=100.0, value=20.0, step=5.0, key="sig_cum_max",
                                         disabled=not sig_req_cum_pct)
        with col_s3:
...
with tab3:
    ...
    if selected_codes and st.button("📊 生成图表", type="primary", use_container_width=True):
        chart_results = []
        ...
        for ts_code in selected_codes:
            ...
            with st.spinner(f"正在获取 {ts_code} 的数据并生成图表..."):
                ...
                st.image(daily_buf, caption=f"{stock_name} 日线图", use_container_width=True)
                ...
                if not weekly_df.empty:
                    ...
                    st.image(weekly_buf, caption=f"{stock_name} 周线图", use_container_width=True)
