...
from data_service import (
    ...
    get_stock_kline_data, get_stock_weekly_kline
)
...
with st.sidebar:
    ...
    token_input = st.text_input(
        ...
        type="password",
        help="请在 https://tushare.pro 获取你的API Token"
    )
    if token_input:
        st.session_state.tushare_token = token_input

    # 检查Token是否已配置
    token_from_secrets = st.secrets.get("TUSHARE_TOKEN", "")
    has_token = bool(token_from_secrets and token_from_secrets != "your_tushare_token_here") or bool(token_input)
    if has_token:
        st.success("Token已配置 ✓")
    else:
        st.warning("请先配置Tushare Token")
...
with tab1:
    ...
    if not has_token:
        st.info("👈 请先在侧边栏配置Tushare API Token")
    else:
        # 参数设置区域
